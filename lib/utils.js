...
export type Pick<FromType, Properties: { [string]: 0 }> = $Exact<$ObjMapi<Properties, <K, V>(k: K, v: V) => $ElementType<FromType, K>>>;
...
export function childrenEqual(a: ReactChildren, b: ReactChildren): boolean {
  return isEqual(
    React.Children.map(a, c => {
      return {key: c?.key, props: c.props['data-grid']}
    }),
    React.Children.map(b, c => {
      return {key: c?.key, props: c.props['data-grid']}
    }),
  )
}
...
export function synchronizeLayoutWithChildren(
  ...
  React.Children.forEach(children, (child: ReactElement<any>) => {
    ...
    if (exists) {
      layout.push(cloneLayoutItem({...exists, ...child.props["data-grid"]}))
    } else {
